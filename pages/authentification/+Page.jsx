...
export default function PageAuth() {
  ...
  const handleLogin = async (e) => {
    ...
    if (res.ok) {
      ...
      if (userProfil.active === true) {
        navigate("/authentification/profile-selector");
      } else {
  ...
  const handleSignIn = async (e) => {
    ...
    const response = await fetch(`${import.meta.env.VITE_API_URL}/signup?username=${formSign.username}`);
    if (response.ok) {
      const userAll = await response.json();
      saveUserId(userAll[0].id);
      navigate("/authentification/offres");
    } else {
      const errorData = await res.json();
        ...
        >
          ...
          >
            ...
            <input
              ...
              required
            />
            <a href="#" className="text-gray-600 text-xs md:text-sm mt-2">
              Mot de passe oublié ?
            </a>
        ...
        <div className="absolute top-0 w-1/2 h-full transition-all duration-600 ease-in-out transform z-2">
          ...
          >
            ...
            <div className="text-left text-sm text-gray-600">
              <p
                className={
                  passwordCriteria.length ? "text-green-500" : "text-red-600"
                }
              >
              ...
              <p
                className={
                  passwordCriteria.uppercase ? "text-green-500" : "text-red-600"
                }
              >
              ...
              <p
                className={
                  passwordCriteria.lowercase ? "text-green-500" : "text-red-600"
                }
              >
              ...
              <p
                className={
                  passwordCriteria.number ? "text-green-500" : "text-red-600"
                }
              >
              ...
              >
                {passwordCriteria.specialChar ? "✔️" : "❌"} Un caractère
                spécial
              </p>
            </div>
            <div className="w-full max-w-xs mr-24 md:mx-auto transform scale-50 md:scale-100 lg:scale-105">
