...
export default function Navbar(pageContext) {
  ...
  const getLogo = () => {
    const classes = pageContext.className.split(" ");
    const bgColor = classes.find(
      (cls) =>
        darkColors.includes(cls) ||
        lightColors.includes(cls) ||
        blacksColors.includes(cls),
    );
  ...
  const getLogoResponsive = () => {
    const classes = pageContext.className.split(" ");
    const bgColor = classes.find(
      (cls) => darkColors.includes(cls) || lightColors.includes(cls),
    );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const listeData = await fetch(
          import.meta.env.VITE_API_URL + "/posts/listes",
        ).then((res) => res.json());
        setData(listeData);
      } catch (error) {
  ...
  return (
    <div className={pageContext.className}>
      <nav
        className={`${
          isOpen
      ...
      >
          ...
          <div>
            <a href={"/accueil"}>
              ...
              <div className="flex w-20 md:hidden">
                <img src={getLogoResponsive()} alt="logoOne" />
              </div>
            </a>
          </div>
        </section>
        ...
        >
          ...
          <ul
            className={`${isOpen ? " " : "flex"} gap-5 list-none items-center`}
          >
            {navlinks.map((link) => {
              return (
                <li
              ...
              {avatarUrl && profileName ? (
                <div className="relative">
                  <a href="/profile">
                    <img
                      src={
                        avatarUrl || "/assets/avatar/narutoShippudenAvatar.webp"
                      }
                      alt={profileName}
