    ...
    const fetchData = async () => {
      try {
        const classData = await fetch(
          import.meta.env.VITE_API_URL + "/posts/listes/classiques",
        ).then((res) => res.json());
        setDataClassique(classData);
        setLoading(false);
      } catch (error) {
        console.error(
          "Erreur pour recuperer la liste des mangas classiques:",
          error,
        );
        setLoading(false);
      }
  ...
  return (
    <>
      <div className="mt-5 text-slate-50 pb-5 text-xl">
        <h2 className="ml-1 ">Les Classiques</h2>
        <Carousel
          swipeable={true}
          ...
          {dataClassique.map((classiques, index) => {
            ...
            return (
              <div
                ...
                key={index}
              >
                <a href={`/manga/${classiques.url}`}>
                  <img
                    src={classiques.image}
                    alt={classiques.name}
