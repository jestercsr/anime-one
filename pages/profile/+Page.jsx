...
export default function PageProfile() {
  ...
  useEffect(() => {
    ...
    const fetchProfiles = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/profiles/${userProfile}`);
        const data = await response.json();
        setProfiles(data);

        const avatarsResponse = await fetch(`${import.meta.env.VITE_API_URL}/avatar`);
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      if (response.ok) {
        alert(
          `L'utilisateur ${selectedAccount.username} à été supprimer avec succès`,
        );
      } else {
  ...
  const handleEditProfiles = () => {
    navigate("/profile/edit-profiles");
  };
  ...
  return (
    <div className="bg-gradient-to-b from-emeralder-900 to-sky-500 text-slate-50">
      <button onClick={handleGoBack} className="text-left text-xl ml-5">
        Retour
      </button>
          ...
          <ul>
            {lien.map((liens, index) => {
              return (
                <li
                  key={index}
                  className="p-3 border-b border-gray-700 hover:underline hover:underline-offset-8"
                >
                  <a href={liens.url}>{liens.name}</a>
                </li>
              );
            })}
