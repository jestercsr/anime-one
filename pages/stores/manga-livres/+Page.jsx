...
export default function PageMangaLivres() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/produits/livres`);
        if (!res.ok) {
            ...
            >
              ...
              <div className="mb-6">
                <Filter title="Character">
                  <div className="grid grid-cols-2">
                    {["Son Goku", "Vegeta", "Naruto", "Luffy"].map((type) => (
                      <label className="block" key={type}>
                        <input type="checkbox" /> {type}
                      </label>
                    ))}
