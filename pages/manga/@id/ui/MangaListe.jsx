    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/mangas/${props}`,
        );
        if (!res.ok) {
    ...
    <>
      {data ? (
        <div className={data.back}>
          <Navbar
            className={data?.navClass}
            liste={data?.search}
            listing={data?.listeSearch}
          />
...
const buildUrl = (url) => {
  ...
  // Sépare : "dragon-ball/films"
  const [id, animation] = url.split("/");

  return `/manga/${id}/${animation}`;
};
