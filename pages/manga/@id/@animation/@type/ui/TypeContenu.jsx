  ...
  return (
    <nav aria-label="breadcrumb" className={color}>
      <ol className="flex items-center space-x-2">
        <li>
          <a href="/accueil" className="hover:underline capitalize">
            Accueil
          </a>
        </li>
          ...
          return (
            <li key={url} className="flex items-center">
              ...
              {isLast ? (
                ...
                <a href={url} className="hover:underline capitalize">
                  {decodeURIComponent(segment).replace(/-/g, " ")}
                </a>
              )}
            </li>
...
export default function TypeContenu({ props, animation, type }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/mangas/${props}/${animation}/${type}`);
        if (!res.ok) {
    ...
    return (
      <>
        {data ? (
          ...
          <div>
            <Navbar
              className="bg-gradient-to-t from-yellow-500 to-orange-500 text-sky-900"
              liste="bg-gradient-to-t from-orange-500 to-yellow-500 text-sky-900 absolute left-0 w-full divide-y-2 divide-slate-50 border-gray-300 mt-1 z-10 list-none"
              listing="cursor-pointer p-2 hover:bg-gray-200 hover:text-orange-500 border-t-0"
            />
          ...
          <div className={data.backNav + ` min-h-screen`}>
              ...
              <table className={data.titre + ` text-sm lg:text-md`}>
                ...
                <tbody>
                  ...
                  <tr className="h-[30px] md:h-auto">
                    ...
                    <td className="capitalize">Saison {data.saison}</td>
                  </tr>
                  <tr className="h-[30px] md:h-auto">
            ...
            <div className="mt-12 mb-8">
              <h2
                className={
                  data.titre +
                  ` uppercase text-2xl border-b-2 flex items-center`
                }
              >
                <MdStars /> Synopsis
              </h2>
              <p className={data.titre + ` text-md mt-5`}>
                {showFullText
                  ? data.description
                  : truncateText(data.description, limitMot)}
              </p>
              <button
                className={
                  data.classNav +
                  `mt-4 ml-4 p-1 hover:underline hover:bg-white/50 `
                }
