...
export default function PageAnimeID({props, animation}) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/mangas/${props}/${animation}`);
        if (!res.ok) {
            ...
            <section className="w-4/5 lg:w-full grid grid-cols-2 gap-1 m-auto items-center xs:grid xs:grid-cols-1 md:grid md:grid-cols-3 md:gap-4 md:items-center xl:grid xl:grid-cols-4">
              {imageSee?.map((select, i) => (
                <div
                  ...
                  key={i}
                >
                  <a href={`/manga/${props}/series/` + select.href}>
                    <img
                      src={select.images}
                      alt={select.href}
            ...
            <section className="w-4/5 lg:w-full grid grid-cols-2 gap-1 m-auto items-center xs:grid xs:grid-cols-1 md:grid md:grid-cols-3 md:gap-4 md:items-center xl:grid xl:grid-cols-4">
              {imageSee?.map((select, i) => (
                <div
                  ...
                  key={i}
                >
                  <a href={`/manga/${props}/scans/` + select.href}>
                    <img
                      src={select.images}
                      alt={select.href}
                      className="w-[80%] md:w-[70%] rounded-2xl"
          ...
          <section className="w-4/5 lg:w-full grid grid-cols-2 gap-1 m-auto items-center xs:grid xs:grid-cols-1 md:grid md:grid-cols-3 md:gap-4 md:items-center xl:grid xl:grid-cols-4">
            {imageSee?.map((select, i) => (
              <div
                ...
                key={i}
              >
                <a href={`/manga/${props}/films/` + select.href}>
                  <img
                    src={select.images}
                    alt={select.href}
                    className="w-[80%] md:w-[70%] rounded-2xl"
