...
export default function Topbar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const listeData = await fetch(`${import.meta.env.VITE_API_URL}/posts/movies/all`).then((res) => res.json());
        setData(listeData);
      } catch (error) {
        console.error("Erreur pour récupérer la liste des films:", error);
      } finally {
        setLoading(false);
      }
    };
        ...
        {searchTerm && filteredData.length > 0 && (
          <ul className="bg-gray-500 text-slate-50 absolute left-3 top-8 w-[90%] divide-y-2 divide-slate-50 border-gray-300 mt-2 z-10 list-none">
            {filteredData.map((mangas, i) => (
              ...
              >
                <a
                  href={`/manga/${mangas.manga}/${mangas.type}/${mangas.url}`}
                >
                  {mangas.name}
                </a>
              </li>
