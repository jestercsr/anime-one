...
export default function PageSeriesAdmin() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const serieData = await fetch(import.meta.env.VITE_API_URL + "/posts/listes/series").then((res) => res.json());
        setDataSerie(serieData);
        setLoading(false);
      } catch (error) {
        console.error("Erreur pour recuperer la liste des séries:", error);
        setLoading(false);
      }
    };
    fetchData();
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-indigo-600 text-slate-50">
            ...
            return (
              ...
              >
                ...
                <div className="p-6 justify-center">
                  <h2 className="text-lg font-bold text-gray-900 line-clamp-2 lg:h-[60px]">
                    {serie.name}
                  </h2>
                  <h3 className="text-md text-gray-600 line-clamp-1">
                    {serie.auteur}
                  </h3>
                  <h3 className="text-md text-gray-600 line-clamp-1">
                    Studio: {serie.studio}
