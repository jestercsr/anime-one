...
export default function ClothesBySlug({ props }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/collection/${props}`);
        if (!res.ok) {
  ...
  useEffect(() => {
    const fetchProduit = async () => {
      try {
        const res = await fetch(`/api/produits/clothes/${props}`);
        if (!res.ok) {
          throw new Error("Erreur lors de la recupération des données");
        }
          ...
          <div className="mb-3">
            ...
            >
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2">
                  <label className="block">
                    <input type="checkbox" /> Figurine Action
                  </label>
                  <label className="block">
                    <input type="checkbox" /> Figurine Non-Scale
                  </label>
                  <label className="block">
              ...
              <Filter title="Taille">
                <div className="grid grid-cols-2">
                  {['Taille unique', 'XS', 'S', 'M', 'L', 'XL', 'XXL'].map((size) => (
              ...
              <div className="mb-6">
                ...
                {['Rouge', 'Bleu', 'Vert', 'Noir', 'Blanc', 'Jaune', 'Rose', 'Gris', 'Multicolor'].map((color) => {
                  const colorClass = {
                    Rouge: 'bg-red-500',
                    Bleu: 'bg-blue-500',
                    Vert: 'bg-green-500',
                  ...
                  return (
                    <label key={color} className="relative inline-block">
                      ...
                      <span
                        className={`block w-5 h-5 rounded-full ${color !== 'Multicolor' ? colorClass[color] : ''} cursor-pointer transition-all duration-300`}
                        style={color === 'Multicolor' ? { background: 'linear-gradient(90deg, red, yellow, green, blue)' } : {}}></span>
              ...
              <Filter title="Type">
                <div className="grid grid-cols-2">
                  {['Casquette', 'Montre', 'Figurine', 'Sac à dos'].map((type) => (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {data.map((product, index) => {
                        return (
                          ...
                          >
                            <Link href={`/stores/collections/${product.manga}/${product.url}`}>
                              <img src={product.image[0]} className="w-full h-[200px] md:h-[250px] lg:h-[300px] rounded-2xl" />
                            </Link>
